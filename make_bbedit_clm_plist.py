...
def create_code(data):
    keywordlist = '\n'.join(['\t\t<string>{!s}</string>'.format(kw) for kw in sorted(data['keywords'] | data['reserved'] | data['blocks'])])
    namelist = '\n'.join(['\t\t<string>{!s}</string>'.format(kw) for kw in sorted(data['distributions'] | data['functions'] | data['types'])])
